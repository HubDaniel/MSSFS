...
double lv(vec& par, List& Y, List& X, List& Z, double& init, List& hypers, double& lambda);
...
double lv(vec& par, List& Y, List& X, List& Z, double& init, List& hypers, double& lambda){
  ...
  // we are maximizing likelihood, so the lambda should be negative.
  if(init==0){
    logL_L2 = logL - lambda * ( pow(alpha1,2) + as_scalar(sum(pow(beta1,2),0)) + pow(zeta1,2)  );
  }else{
    logL_L2 = logL - lambda * ( pow(alpha1,2) + as_scalar(sum(pow(beta1,2),0)) );
  }
...
return wrap(lv(par, Y, X, Z, init, hypers, lambda));
'
