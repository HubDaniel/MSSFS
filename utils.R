...
hyperSetting <- function(lag=3,nS=300,nT=101,Ws="exp.ave",xL = 2,max_rel_change = 1e-3){
...
gdataus <- function(num_miss=round(hyperSetting()$timeL/3)){
  ...
  ### hyperparameters
  timeL <- hyperSetting()$timeL # num of observations
  thetaL <- hyperSetting()$thetaL  # num of biomarkers : temperature, O2 saturation, blood pressure: temperature negative O2, pressure negative O2, temperature positive pressure
  xL <- hyperSetting()$xL
  ws = as.numeric(hyperSetting()$ws)
  ...
  alpha0 <- -3#-3.7051           # intercept for p_01 in the switch equation
  alpha1 <- 0.2#4#1.0560              # intercept for p_11 in the switch equation
  zeta1 <- 0.3#-0.3#2.6096 
  
  beta0 <- c(0.15,-0.2) #c(0.1636,-0.1906)
  beta1 <- c(-0.8,0.5) #c(-0.8652,0.8447) 
  ## history of the system and their coefficients
  lagL0 <- hyperSetting()$lagL0       # time lags for p_01 when generate data
  lagL1 <- hyperSetting()$lagL1       # time lags for p_11 when generate data
  ...
  if(hyperSetting()$have_miss){
    missing_t = sort(sample(1:hyperSetting()$timeL, num_miss))
    Y_obs[,missing_t] = NA
    Ii_obs[missing_t] = NA
  }
  ...
  plot(Y[1,],type="b",ylab="Observations", xlab="Time (Days)")
  ...
  plot(Y_5_P[1,],type="b",ylab="Observations", xlab="Time (Days)", 
       main = expression(paste("Positive Feedback with ",delta,"=5")))
  abline(v=which(I_5_P==1),col="red",lty=3)
  plot(Y_10_P[1,],type="b",ylab="Observations", xlab="Time (Days)",
       main = expression(paste("Positive Feedback with ",delta,"=10")))
  abline(v=which(I_10_P==1),col="red",lty=3)
  plot(Y_5_N[1,],type="b",ylab="Observations", xlab="Time (Days)",
       main = expression(paste("Negative Feedback with ",delta,"=5")))
  abline(v=which(I_5_N==1),col="red",lty=3)
  plot(Y_10_N[1,],type="b",ylab="Observations", xlab="Time (Days)",
       main = expression(paste("Negative Feedback with ",delta,"=10")))
  abline(v=which(I_10_N==1),col="red",lty=3)
